...
const PinIcon = ({ status, letter, onClick }) => {
  ...
  return (
    <i className={"pinIcon " + className} onClick={onClick}>
      {letter}
    </i>
  );
};

const Memo = ({ memo, onPinChanged, onDeleted }) => {
  ...
  return (
    <li className="memo">
      ...
      <PinIcon
        ...
        letter="W"
        onClick={() => {
          toggleWeeklyPin(memo);
        }}
      />
      <span className="content">{memo.text}</span>
      <a className="btn-delete" onClick={onDeleteClicked} href="#delete">
        {deleteIcon}
      </a>
    </li>
  );
